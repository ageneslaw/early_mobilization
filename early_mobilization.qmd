...
df_vent_pt_any %>% 
     ...
     modify_header(label = '**Patients requiring IMV**')
...
df_tab1 <- df_tab1 %>% 
     ...
     group_by(mrn) %>% 
     arrange(mrn, icu_start_date) %>% 
     mutate(icu_n = row_number(),
            multi_icu = if_else(any(icu_n > 1), TRUE, FALSE)) %>% 
     ungroup()

tbl_vars <- c('agem', 'female_sex', 'vent24', 'early_pt', 'multi_icu', 'cticu')

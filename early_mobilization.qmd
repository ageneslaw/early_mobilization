...
df_pt_timing <- df_pt_picu %>%
     mutate(early_pt = if_else(recorded_time %within% int0_72 & pt_time > 0, TRUE, FALSE, FALSE),
            any_pt = if_else(recorded_time > icu_start_date & pt_time > 0, TRUE, FALSE, FALSE),
            icu_pt = if_else(recorded_time %within% interval(icu_start_date, icu_stop_date) & pt_time > 0, TRUE, FALSE, FALSE),
            pt0_24 = if_else(recorded_time  %within% int0_24 & pt_time > 0, TRUE, FALSE, FALSE),
            pt24_48 = if_else(recorded_time %within% int24_48 & pt_time > 0, TRUE, FALSE, FALSE),
            pt48_72 = if_else(recorded_time  %within% int48_72 & pt_time > 0, TRUE, FALSE, FALSE),)
...
df_pt_counts <- df_pt_timing %>%  
     ...
     summarize(n_early_pt = sum(early_pt), 
               ...
               early_pt_time = sum(if_else(early_pt, pt_time, 0, 0)),
               icu_pt_time = sum(if_else(icu_pt, pt_time, 0, 0)),
               any_pt_time = sum(if_else(any_pt, pt_time, 0, 0)),
               .groups = 'drop') %>% 
...
tbl_summary(df_pt_counts,
            include = starts_with('n_'),
